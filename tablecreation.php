<?php 
	...
	// $result = $conn->query("SELECT * FROM `pe_qpapers` WHERE `QpId`='$QpId'");
	$result = $conn->query("SELECT * FROM `pe_qpapers` WHERE `CourseId`='$QpId'");
	...
	$result = $conn->query("SELECT * FROM `pe_marks` WHERE `StudId`='{$studid}' AND `QpId`='{$QpId}' AND `MarksPerQ`>='0'");
	if ($result->num_rows > 0) {
	...
	$result = $conn->query("SELECT * FROM `pe_marks` WHERE `StudId`='{$studid}' AND `QpId`='{$QpId}' ");
	if ($result->num_rows > 0) {
	...
	*/
	...
	foreach($marks as $qno => $subqarr) {
		...
		foreach ($subqarr as $subqno => $maxmarks) {
			...
			$qid="";
			$result1 = $conn->query("SELECT `QId` FROM `pe_qpapers` WHERE `QNo`='$qno' AND `SubQNo`='$subqno' AND `CourseId`='$QpId'");
