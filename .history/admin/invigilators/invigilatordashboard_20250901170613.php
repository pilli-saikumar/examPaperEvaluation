...
$query = "SELECT in.id as invigilator_id, in.invigilator_name, in.invigilator_email,  in.exam_id, in.subject_id, pe_subjects.subject_name,pe_question_paper.question_paper FROM `pe_assign_candidates` as in JOIN pe_subjects ON in.subject_id = pe_subjects.id JOIN pe_question_paper ON in.question_paper_id = pe_question_paper.id WHERE in.invigilator_id = " . (int)$_GET['uid'] . "";
