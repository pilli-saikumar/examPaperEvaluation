                           ...
                           <input type="number" class="form-control" name="candidate_assign_count" id="candidate_assign_count" required>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-4" id="assignedCount">
                                        <div id="assignedCount" class="p-3"></div>
                                    </div>
            ...
            if (subject_id && exam_id) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        // Function to update the assignment count display
                        function updateAssignmentCount() {
                            $('#assignedCount').text("Assigned: " + currentAssignedCount + "/" + totalCandidates);
                        }
                        ...
                        $('#selectAllInvigilators').on('change', function() {
                            let remaining = totalCandidates - currentAssignedCount;
                            let canCheckAll = $('.invigilator-checkbox:not(:checked)').length <= remaining;
                            
                            if (this.checked && !canCheckAll) {
                                alert('Cannot select all invigilators as it would exceed the total candidate count of ' + totalCandidates);
                                this.checked = false;
                                return;
                            }
                            
                            $('.invigilator-checkbox').prop('checked', this.checked);
                            currentAssignedCount = this.checked ? $('.invigilator-checkbox').length : 0;
                            updateAssignmentCount();
                        });
                        ...
                        $(document).on('change', '.invigilator-checkbox', function() {
                            ...
                            if (this.checked && currentAssignedCount >= totalCandidates) {
                                alert('You have reached the maximum number of candidates that can be assigned (' + totalCandidates + ')');
                                $(this).prop('checked', false);
                                return;
                            }
                            
                            // Update the current assigned count
                            currentAssignedCount = checked;
                            updateAssignmentCount();
