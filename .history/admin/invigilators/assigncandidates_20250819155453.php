  ...
  <script>
    ...
    $(document).ready(function(){
        ...
        $('#invigilator_id').multiselect({
            ...
            nonSelectedText: 'Select Invigilator',
...
//                     // Update candidate count
//                     // $('#candidateCount').text("Total Candidates: " + parts[1].trim());
//                 }
//             },
//             error: function(xhr, status, error) {
//                 console.error("AJAX Error:", status, error);
//                 $select.html('<option value="">Error loading invigilators</option>');
//                 $('#candidateCount').text('Total Candidates: 0');
//             }
//         });
//     } else {
...
// This function should be called on a change event from your subject/exam dropdowns
function getInvigilators() {
    var subject_id = $('#subject_id').val();
    var exam_id = $('#exam_id').val();
    ...
    if (subject_id && exam_id) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (parts.length === 2) {
                    // Check if multiselect plugin exists and if it has been initialized before
                    // This is crucial for avoiding the TypeError on subsequent calls
                    if ($.fn.multiselect && $select.hasClass('multiselect')) {
                         $select.multiselect('destroy');
                    }
                    
                    // Update the select element's options with the new data
                    $select.html(parts[0]);
                    
                    // Reinitialize the multiselect plugin on the newly populated element
                    $select.multiselect({
                        buttonWidth: '100%',
                        includeSelectAllOption: true,
                        enableFiltering: true,
                    ...
                    // Your other logic to update candidate count and show the element
                    $('#candidateCount').html("Total Candidates: " + parts[1]);
                    $('#invigilator').show();
                } else {
                    // Handle case where response format is incorrect
                    $select.html('<option value="">No invigilators found</option>');
                    $('#candidateCount').html('Total Candidates: 0');
                }
            },
            error: function(xhr, status, error) {
        ...
        // Handle case where subject or exam is not selected
        var $select = $('#invigilator_id');
        if ($.fn.multiselect && $select.hasClass('multiselect')) {
            $select.multiselect('destroy');
        }
...
// Ensure this function is called from an event, such as a change on your subject or exam dropdowns.
// Example: $('#subject_id, #exam_id').on('change', getInvigilators);
</script>
