...
$result = mysqli_query($conn, "SELECT s.subject_name,q.question_paper, * FROM `pe_assign_candidates` JOIN pe_subjects.id=pe_assign_candidates.subject_id JOIN pe_question_paper.id=pe_assign_candidates.question_paper_id  group by invigilator_id ORDER BY id DESC");
