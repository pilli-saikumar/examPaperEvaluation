...
function getInvigilators() {
        ...
        // Ensure both subject and exam are selected
        if (subject_id && exam_id) {
            var $select = $('#invigilator_id');
            
            // Show loading state and destroy previous multiselect instance
            if ($select.hasClass('multiselect')) { // Check if multiselect was already initialized
                $select.multiselect('destroy');
            }
            $select.html('<option value="">Loading invigilators...</option>');
            // Optionally, hide the #invigilator div while loading, then show it on success
            $('#invigilator').hide(); 

            // Make AJAX request
            $.ajax({
                url: 'get_invigilators.php', // Your PHP endpoint to get invigilators
                type: 'POST',
                data: { 
                    subject_id: subject_id, 
                    exam_id: exam_id 
                ...
                success: function(response) {
                    ...
                    console.log(response); // For debugging: Check what your PHP returns
                    
                    if (parts.length === 2) {
                        // Update options in the select box
                        $select.html(parts[0]);
                        
                        // Reinitialize multiselect with new options
                        // This is where the plugin is actually applied/reapplied
                        $select.multiselect({
                            buttonWidth: '100%',
                            includeSelectAllOption: true,
                            enableFiltering: true, // As per your latest AJAX code
                            maxHeight: 300,
                            numberDisplayed: 2
                        });
                        ...
                        // Show the invigilator div once populated and styled
                        $('#invigilator').show(); 
                    } else {
                        // Handle unexpected response format
                        $select.html('<option value="">No invigilators found or unexpected response</option>');
                        $('#candidateCount').html('Total Candidates: 0');
                        $('#invigilator').show(); // Still show, but with message
                    }
                },
                error: function() {
                    $select.html('<option value="">Error loading invigilators</option>');
                    $('#candidateCount').html('Total Candidates: 0');
                    $('#invigilator').show(); // Still show, but with error message
                }
            });
        } else {
            // Reset dropdown if subject or exam is not selected
            var $select = $('#invigilator_id');
            ...
            $('#candidateCount').html('Total Candidates: 0');
            $('#invigilator').show(); // Ensure div is visible to show prompt
        }
    }

    // You would trigger getInvigilators() based on changes to subject_id or exam_id
    // Example: When subject or exam dropdown changes
    $('#subject_id, #exam_id').on('change', function() {
        getInvigilators();
    });

    // Initial call if you want it to load on page load if values are already present
    // Or, you can leave it to be triggered only by user selection.
    // getInvigilators(); 
