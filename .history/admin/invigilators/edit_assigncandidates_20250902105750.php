...
$getInvigilatorData    =   mysqli_query($conn,'SELECT * FROM pe_assign_candidates WHERE invigilator_id = '.$uid.' AND exam_id = '.$exam_id.' AND subject_id = '.$subject_id.' GROUP BY invigilator_id ORDER BY id DESC'  );

$invigilator = [];
if(mysqli_num_rows($getInvigilatorData) > 0) {
    $invigilator = mysqli_fetch_assoc($getInvigilatorData);  // Get first row since we're using GROUP BY invigilator_id
}
...
$subject = array();
$getsubject = mysqli_query($conn,'SELECT * FROM pe_subjects WHERE id = '.$subject_id.'');
if(mysqli_num_rows($getsubject) > 0){
    while($subjectData   =   mysqli_fetch_assoc($getsubject)){
        $subject[]       =    $subjectData;
    }
}
