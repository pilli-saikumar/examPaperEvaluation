...
//$result = mysqli_query($conn, "SELECT s.subject_name,q.question_paper, * FROM `pe_assign_candidates` ac JOIN pe_subjects as s ON s.id=ac.exam_id JOIN pe_question_paper as q ON q.id=pe_assign_candidates.subject_id  group by ac.invigilator_id,s.subject_name ORDER BY id DESC");
$result = mysqli_query($conn, "
    SELECT 
        ac.invigilator_id,
        s.subject_name,
        q.question_paper,
        COUNT(DISTINCT ac.candidate_id) as assigned_count
    FROM pe_assign_candidates ac
    JOIN pe_subjects s ON s.id = ac.subject_id
    JOIN pe_question_paper q ON q.id = ac.exam_id
    GROUP BY ac.invigilator_id, s.subject_name, q.question_paper
    ORDER BY ac.id DESC
");
