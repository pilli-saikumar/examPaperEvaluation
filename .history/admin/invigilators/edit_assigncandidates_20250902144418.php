...
if(isset($_POST['saveChanges'])){
    ...
    $existingdata = mysqli_query($conn,'SELECT * FROM pe_assign_candidates WHERE invigilator_id = '.$current_invigilator.' AND exam_id = '.$exam_id.' AND subject_id = '.$subject_id.' GROUP BY invigilator_id ORDER BY id DESC'  );

    if(mysqli_num_rows($existingdata) > 0){
        $existingdata = mysqli_fetch_assoc($existingdata);
    }
    echo "<pre>";
