...
// }
$currentexam = mysqli_query($conn,'SELECT * FROM pe_assign_candidates WHERE invigilator_id = '.$uid.' AND exam_id = '.$exam_id.' AND subject_id = '.$subject_id.' GROUP BY invigilator_id ORDER BY id DESC');
if(mysqli_num_rows($currentexam) > 0){
    $invigilator = mysqli_fetch_assoc($currentexam);
}
...
if(isset($_POST['saveChanges'])){
    ...
    // 'isii' specifies the types of variables: integer, string, integer, integer
    mysqli_stmt_bind_param($stmt, 'isii', $new_invigilator, $current_invigilator, $exam_id, $subject_id);

    // Execute the statement
    if (mysqli_stmt_execute($stmt)) {
        // You can check the number of rows affected
