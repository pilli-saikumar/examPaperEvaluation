...
$getCandidateData    =   mysqli_query($conn,'SELECT c.candidate_name,c.candidate_email,c.id,c.candidate_phone  from pe_assign_candidates as ca JOIN pe_candidates as c ON c.id=ca.candidate_id where ca.invigilator_id ='.$_GET['uid'].' AND ca.subject_id ='.$_GET['sid'].' AND ca.exam_id ='.$_GET['eid'].' group by c.id');
