...
$result = mysqli_query($conn, "SELECT s.subject_name,q.question_paper, * FROM `pe_assign_candidates` ac JOIN pe_subjects as s ON s.id=ac.exam_id JOIN pe_question_paper as q ON q.id=pe_assign_candidates.subject_id  group by ac.invigilator_id,s ORDER BY id DESC");
