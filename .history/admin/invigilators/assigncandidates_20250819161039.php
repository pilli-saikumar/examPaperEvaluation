    ...
    if (subject_id && exam_id) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                // Split the response
                var parts = response.split('||COUNT||');
                var optionsHtml = parts[0].trim();   // The <option> tags
                var count = parts[1] ? parts[1].trim() : '0'; // Total candidates

                // Update select
                $select.html(optionsHtml);
                $select.multiselect('rebuild'); // Re-initialize multiselect (important!)

                // Update candidate count
                $('#candidateCount').text("Total Candidates: " + count);
            },
            error: function() {
                $select.html('<option value="">Error loading invigilators</option>');
                $('#candidateCount').text('');
