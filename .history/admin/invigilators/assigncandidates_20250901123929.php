...
if (isset($_POST['saveChanges']) && $_POST['saveChanges'] == 'Submit') {
    ...
    $deleteQuery = mysqli_query($conn, "DELETE ac FROM pe_assign_candidates ac 
    ...
    if ($assign_candidates == 'individual') {
        ...
        $assign_candidate_counts = $_POST['assign_candidate_count'];
        $invigilator  = [];
        foreach ($invigilator_ids as $invigilator_id) {
            $getinvigilatorData    =   mysqli_query($conn, 'SELECT id FROM pe_invigilators WHERE id = ' . $invigilator_id . ' AND subject = ' . $subject_id . ' AND exam_id = ' . $exam_id . ' ');
        ...
        $assigned = 0;
                       foreach ($invigilator_ids as $key => $invigilatorId) {
                            $toAssign = intval($assign_candidate_counts[$key]);

                            for ($i = 0; $i < $toAssign; $i++) {
                                if (isset($candidates[$assigned])) {
                                    $candidateId = $candidates[$assigned];
                                    $insertQuery = "
                                        INSERT INTO pe_assign_candidates (candidate_id, invigilator_id, subject_id,assign_count,exam_id) 
                                        VALUES ('$candidateId', '$invigilatorId', '$subject_id','$toAssign','$exam_id')
                                    ";
        ...
        // echo "<pre>";
        // print_r($getinvigilatorData);die;
        ...
        $getInvigilators = mysqli_query($conn, "SELECT id FROM pe_invigilators 
                                              WHERE subject = $subject_id AND exam_id = $exam_id
                                              ORDER BY id");
        ...
        if ($totalInvigilators > 0) {
            ...
            foreach ($invigilators as $index => $invigilatorId) {
                ...
                for ($i = 0; $i < $toAssign; $i++) {
                    if (isset($candidates[$assigned])) {
                        $insertQuery = "INSERT INTO `pe_assign_candidates`(`candidate_id`, `invigilator_id`, `subject_id`, `exam_id`, `assign_count`) 
                                      VALUES ('" . $candidates[$assigned] . "', '$invigilatorId', '$subject_id', '$exam_id', '$toAssign')";
                        mysqli_query($conn, $insertQuery);
                        $assigned++;
                    }
                }
...
<div class="container-scroller">
    ...
    <script>
        $(document).ready(function() {
            // Initialize the multiselect plugin on page load
            //  initMultiselect();
        ...
        //         buttonWidth: '100%',
        //         includeSelectAllOption: true,
        //         enableFiltering: true, // It's better to enable filtering for a long list
        //         maxHeight: 300,
        //         numberDisplayed: 2
        //     });
        // }
        ...
        function getsubjectName() {
            ...
            if (exam_name) {
                $.ajax({
                    ...
                    success: function(response) {
                        document.getElementById('subject_id').innerHTML = '<option value="">Select Subject</option>' + response;
            ...
            if (subject_id && exam_id) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (candidateCount == 0) {
                        $('#msg').show();
                        $('#msg').html("No candidates found for this exam");
                        $('#invigilator').html(''); // Clear invigilator list if no candidates
                       
                      } else {
                        $('#msg').hide(); // Hide the message if there are candidates
                        $('#invigilator').html(optionsHtml);
                        $('#candidateCount').html("👤 Total Candidates: " + candidateCount);
                                 
                        // Rest of your code for handling checkboxes
                        $('#selectAllInvigilators').on('change', function() {
                            $('.invigilator-checkbox').prop('checked', this.checked);
                        });

                        $(document).on('change', '.invigilator-checkbox', function() {
                            var total = $('.invigilator-checkbox').length;
                           
                            var checked = $('.invigilator-checkbox:checked').length;
                           
    
    // Show/hide the count input based on checkbox state
    //$countInput.toggle($checkbox.is(':checked'));
                           
                            $('#selectAllInvigilator').prop('checked', total === checked);
                        });
        ...
        $(document).on('input', '.assign_candidate_count', function() {
          
    const invigilatorId = $(this).attr('id').replace('assign_candidate_count_', '');
    const count = $(this).val();
  var totalcandidatecount = $('#candidateCount').text().split(': ')[1];

    
   
    
    // Update the corresponding checkbox if count is greater than 0
    const $checkbox = $(`#invigilator_${invigilatorId}`);
    if (count > 0) {
        $checkbox.prop('checked', true);
    }
    
    // Calculate and display total assigned count
    let totalAssigned = 0;
    $('.assign_candidate_count').each(function() {
        totalAssigned += parseInt($(this).val()) || 0;
    });
    
    if(totalAssigned > totalcandidatecount){
        $('#msg').show();
        $('#msg').html("Total assigned count cannot be greater than total candidate count");
        $('#submit_button').hide();
    }
    else{
        $('#msg').hide();
    ...
    // Update UI to show total assigned count (optional)
    $('#totalAssignedCount').text(`Total Assigned: ${totalAssigned}`);
});
    </script>
