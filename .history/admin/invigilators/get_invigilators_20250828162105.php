...
if(isset($_POST['exam_id']) && $_POST['exam_id'] != ''){
   ...
   // $getInvigilatorData = mysqli_query($conn, 'SELECT * FROM `pe_invigilators` WHERE subject ='.$exam_id.' ORDER BY id DESC');
    $getInvigilatorData = mysqli_query($conn, 'SELECT * FROM `pe_invigilators` WHERE  exam_id ='.$exam_id.' AND subject ='.$subject_id.'  ORDER BY id DESC');
    $output = '';
...
if ($getInvigilatorData) {
    ...
    $output .= "<input type='number' id='assignedCount' class='form-control ml-2' placeholder='Assign Candidate Count' value='' required id='assignedCount'>";
    $output .= "</div>";

    $output .= "<div class='invigilator-list' style='max-height: 300px; overflow-y: auto;'>";
    while ($invigilator = mysqli_fetch_assoc($getInvigilatorData)) {
        ...
        $output .= "<input class='form-control ml-2' type='number' name='assign_candidate_count[]' value='' placeholder='Assign Candidate Count' id='assign_candidate_count_".$invigilator['id']."'>";
        $output .= "</div>";
    }
    $output .= "</div></div>";
}
