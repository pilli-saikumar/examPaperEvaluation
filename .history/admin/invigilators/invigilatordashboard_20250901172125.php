...
$result = mysqli_query($conn, "SELECT * FROM `pe_assign_candidates` as ac JOIN `pe_invigilators` as i ON i.id = ac.invigilator_id JOIN `pe_subjects` as s ON s.id = ac.subject_id JOIN `pe_candidates` as c ON c.id = ac.candidate_id group by invigilator_id ORDER BY id DESC");
