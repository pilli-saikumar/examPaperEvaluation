                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 col-sm-6 col-lg-6">
                              <div class="form-group">
                                    ...
                                    <?php 
if(isset($invigilator['exam_id']) && !empty($invigilator['exam_id'])) {
    $getSubject = mysqli_query($conn, "SELECT * FROM `pe_question_paper` 
                                     WHERE exam_id = " . $invigilator['exam_id'] . " 
                                     AND id = " . $invigilator['subject']);
    
    if(mysqli_num_rows($getSubject) > 0) {
        $subjectData = mysqli_fetch_assoc($getSubject);
        $selectedSubjectId = $subjectData['id'];
        $selectedSubjectName = $subjectData['question_paper'];
    }
}
?>
...
<select class="form-control" name="subject_id" id="subject_name" required>
    ...
    <?php if(isset($selectedSubjectId) && !empty($selectedSubjectId)): ?>
        <option value="<?php echo $selectedSubjectId; ?>" selected>
            <?php echo htmlspecialchars($selectedSubjectName); ?>
        </option>
    <?php endif; ?>
</select>
                                    <!-- <select class="form-control" name="subject_id" id="subject_name" required>
                                     <?php if(isset($invigilator['exam_id']) && !empty($invigilator['exam_id']) && isset($invigilator['id']) && !empty($exam['id'])){ 
                                              $getSubjectName   =   mysqli_query($conn,'SELECT * FROM `pe_question_paper` where exam_id='.$exam['id']. ' AND  id='.$exam['id'].' ');
                                       
                                                if(mysqli_num_rows($getSubjectName) > 0){
