                        ...
                        <div class="card">
                           ...
                           <input type="number" class="form-control" name="candidate_assign_count" id="candidate_assign_count" required>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-4" id="assignedCount">
            ...
            if (subject_id && exam_id) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        let totalCandidates = parseInt(candidateCount);
                        let remainingCandidates = totalCandidates;
                        
                        // Function to update the remaining candidates display
                        function updateRemainingCount() {
                            let totalAssigned = 0;
                            $('.assign-candidate-input:visible').each(function() {
                                totalAssigned += parseInt($(this).val()) || 0;
                            });
                            remainingCandidates = totalCandidates - totalAssigned;
                            $('#remainingCandidates').html(`Remaining Candidates: <strong>${remainingCandidates}</strong>/${totalCandidates}`);
                        ...
                        $('#selectAllInvigilators').on('change', function() {
                            $('.invigilator-checkbox').prop('checked', this.checked);
                            $('.assign-candidate-input').toggle(this.checked);
                            if (!this.checked) {
                                $('.assign-candidate-input').val('');
                            }
                            updateRemainingCount();
                        });

                        // Handle individual invigilator checkbox
                        $(document).on('change', '.invigilator-checkbox', function() {
                            const $input = $(this).closest('.form-check').find('.assign-candidate-input');
                            ...
                            const total = $('.invigilator-checkbox').length;
                            const checked = $('.invigilator-checkbox:checked').length;
                            $('#selectAllInvigilators').prop('checked', total === checked);
                        });

                        // Handle candidate count input changes
                        $(document).on('input', '.assign-candidate-input', function() {
                            const value = parseInt($(this).val()) || 0;
                            const maxAllowed = Math.min(remainingCandidates + (parseInt($(this).val()) || 0), totalCandidates);
                            
                            if (value > maxAllowed) {
                                alert(`You can only assign up to ${maxAllowed} more candidates.`);
                                $(this).val(maxAllowed);
                            }
