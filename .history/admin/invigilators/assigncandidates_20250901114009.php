...
if (isset($_POST['saveChanges']) && $_POST['saveChanges'] == 'Submit') {
    ...
    if ($assign_candidates == 'individual') {
        ...
        foreach ($invigilator_ids as $invigilator_id) {
            $getinvigilatorData    =   mysqli_query($conn, 'SELECT id FROM pe_invigilators WHERE id = ' . $invigilator_id . ' AND subject_id = ' . $subject_id . ' AND exam_id = ' . $exam_id . ' ');
            echo "<pre>";print_r($getinvigilatorData);die;
