...
//     i.invigilator_phone,
//     s.subject_name,
//     COUNT(ac.candidate_id) as assigned_candidates_count,
//     GROUP_CONCAT(DISTINCT ac.candidate_id) as candidate_ids,
//     ac.status
// FROM 
//     pe_invigilators i
// LEFT JOIN 
//     pe_assign_candidates ac ON i.id = ac.invigilator_id
// LEFT JOIN
//     pe_subjects s ON i.subject = s.id
// GROUP BY 
//     i.id, i.invigilator_name, i.invigilator_email, i.invigilator_phone, s.subject_name, ac.status
// ORDER BY 
//     i.id DESC";
$query = "SELECT 
    i.id AS invigilator_id,
    ...
    GROUP_CONCAT(DISTINCT c.candidate_name) AS candidate_names
FROM 
    pe_invigilators i
JOIN 
    pe_assign_candidates ac ON i.id = ac.invigilator_id
JOIN 
    pe_candidates c ON ac.candidate_id = c.id
WHERE 
    ac.invigilator_id = 10   -- your invigilator_id
    AND ac.exam_id = 5       -- your exam_id
    AND ac.subject_id = 13   -- your subject_id
GROUP BY 
    i.id, ac.exam_id, ac.subject_id;
