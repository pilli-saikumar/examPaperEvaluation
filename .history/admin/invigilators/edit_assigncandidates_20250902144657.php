...
$getassigneddata    =   mysqli_query($conn,'SELECT * FROM pe_assign_candidates WHERE invigilator_id = '.$uid.' AND exam_id = '.$exam_id.' AND subject_id = '.$subject_id.' GROUP BY invigilator_id ORDER BY id DESC'  );
...
if(isset($_POST['saveChanges'])){
    $new_invigilator = intval($_POST['new_invigilator']); // Convert to integer for safety
    $current_invigilator_id = intval($uid); // Use the original $uid which should contain the current invigilator's ID
    $exam_id = intval($exam_id);
    $subject_id = intval($subject_id);

    // First, verify the current invigilator exists
    $checkInvigilator = mysqli_query($conn, "SELECT id FROM pe_assign_candidates 
                                          WHERE invigilator_id = $current_invigilator_id 
                                          AND exam_id = $exam_id 
                                          AND subject_id = $subject_id 
                                          LIMIT 1");
    
    if(mysqli_num_rows($checkInvigilator) > 0) {
        // Update the invigilator for all candidates assigned to the current invigilator
        $updateQuery = "UPDATE pe_assign_candidates 
                       SET invigilator_id = $new_invigilator 
                       WHERE invigilator_id = $current_invigilator_id 
                       AND exam_id = $exam_id 
                       AND subject_id = $subject_id";
        
        if(mysqli_query($conn, $updateQuery)) {
            $_SESSION['success'] = "Invigilator updated successfully!";
            header("Location: invigilatordashboard.php");
            exit();
        } else {
            $_SESSION['error'] = "Error updating invigilator: " . mysqli_error($conn);
        }
    } else {
        $_SESSION['error'] = "No records found for the current invigilator.";
        header("Location: invigilatordashboard.php");
        exit();
    }
}
