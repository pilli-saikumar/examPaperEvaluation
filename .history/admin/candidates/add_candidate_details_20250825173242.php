...
if (isset($_POST['saveChanges']) && $_POST['saveChanges'] == 'Submit') {
  if (isset($_POST['subject_id']) && !empty($_POST['subject_id'])) {
    ...
    //         //   }
    //         // Try to parse the date into DateTime
    //         $date = DateTime::createFromFormat('Y-m-d', $raw_dob);

    //         if (!$date) {
    //             // Try another common format like "d-m-Y"
    //             $date = DateTime::createFromFormat('d-m-Y', $raw_dob);
    //         }
    //         if (!$date) {
    //             // Try another format "d/m/Y"
    //             $date = DateTime::createFromFormat('d/m/Y', $raw_dob);
    //         }
    ...
    //             echo "Invalid DOB format for candidate: " . $raw_dob;
    //             $dob = null; // or skip this record
    //         }
    ...
    //                   (registration_id, candidate_name, candidate_father_name, candidate_dob, candidate_gender, candidate_email, candidate_phone, subject_id, status,exam_id)
    //                   VALUES 
    //                   ('$registration_id', '$candidate_name', '$candidate_father_name', '$dob', '$candidate_gender', '$candidate_email', '$candidate_phone', '$subject_id_csv', '$status','$exam_id')";
    ...
    if (isset($candidate_details) && $candidate_details['error'] == 0) {
        $filename = $candidate_details['tmp_name'];
        $filetype = pathinfo($candidate_details['name'], PATHINFO_EXTENSION);
        ...
        if (strtolower($filetype) !== 'csv') {
            ...
            if ($handle === FALSE) {
                ...
                // ✅ Step 2: Read and validate headers
                ...
                if ($header === FALSE || array_map('trim', $header) !== $expected_headers) {
                    $error = '❌ Invalid CSV format. Please upload with correct headers: ' . implode(', ', $expected_headers);
                } else {
                    // ✅ Step 3: Connect DB
                    $conn = new mysqli('localhost', 'root', '', 'paper_evaluation_db');
                    if ($conn->connect_error) {
                    ...
                    while (($row = fgetcsv($handle, 1000, ';')) !== FALSE) {
                        ...
                        if (count($row) < count($expected_headers)) {
                            $error .= "⚠️ Skipping row $rowNumber (incomplete data).<br>";
                            continue;
                        }
                        ...
                        // ✅ DOB format check (DD-MM-YYYY or YYYY-MM-DD)
                        $raw_dob = trim($row[3]);
                        $candidate_dob = '0000-00-00';
                        if (preg_match('/^\d{4}-\d{2}-\d{2}$/', $raw_dob)) {
                            $candidate_dob = $raw_dob; // Already YYYY-MM-DD
                        } elseif (preg_match('/^\d{2}-\d{2}-\d{4}$/', $raw_dob)) {
                            $dob_parts = explode('-', $raw_dob);
                            $candidate_dob = $dob_parts[2] . '-' . $dob_parts[1] . '-' . $dob_parts[0];
                        }
                        ...
                        $sql = "INSERT INTO pe_candidates 
                                (registration_id, candidate_name, candidate_father_name, candidate_dob, candidate_gender, candidate_email, candidate_phone, subject_id, status, exam_id)
                                VALUES 
                                ('$registration_id', '$candidate_name', '$candidate_father_name', '$candidate_dob', '$candidate_gender', '$candidate_email', '$candidate_phone', '$subject_id_csv', '$status', '$exam_id')";
    
                        if ($conn->query($sql) === TRUE) {
                            $inserted++;
                        } else {
                            $error .= "❌ Error in row $rowNumber: " . $conn->error . "<br>";
                        }
                    }
                    fclose($handle);
                    $conn->close();
                    ...
                    if ($inserted > 0) {
                        ...
                        $error .= '❌ No candidates were inserted.';
                    }
                }
            }
        }
    } else {
        $error = '❌ Please select subject and upload a valid CSV file.';
    }
