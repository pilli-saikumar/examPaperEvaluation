...
<?php
	...
	if(!empty($_SESSION['FactId'])){
	$factId = $_SESSION['FactId'];
	// $invigilator_id = $_SESSION['FactId'];
	$evalutiontime = date('Y-m-d H:i:s');
	$evaluation_endtime = null;
	$exam_id = $_SESSION['exam_id'];
	$subject_id = $_SESSION['subject_id'];

	$insertinvigilator_data = $conn->query("INSERT INTO `invigilator_evaluated` (`invigilator_id`, `candidate_id`, `evaluation_starttime`, `evaluation_endtime`,`exam_id`,`subject_id`) 
		 VALUES ( '{$factId}', '{$HidId}', '".date('Y-m-d H:i:s')."', '".$evaluation_endtime."', '".$exam_id."', '".$subject_id."')");

   print_r($insertinvigilator_data);
    // $evalutiontime = $conn->query(
    //     "UPDATE `useractions` 
    //      SET `evalution_starttime` = '".date('Y-m-d H:i:s')."', `candidate_id` = '".$HidId."' 
    //      WHERE `userid` = '{$factId}'"
    // );
	...
	$insertevalution_data = $conn->query(
		"INSERT INTO `useractions` (`userid`,`role`,`action`, `candidate_id`, `evalution_starttime`,`ip_address`) 
		 VALUES ('{$factId}', 'invigilator', '{$action}', '".$HidId."', '".date('Y-m-d H:i:s')."','{$ip_address}')"
	);
	
   }
	//fetch courseid and studid
	$result = $conn->query("SELECT `registration_id`,`question_paper_id`,`id` FROM `pe_candidate_answers` WHERE `registration_id`='{$HidId}' LIMIT 1");
	...
	if ($result->num_rows > 0) {
		if($row = $result->fetch_assoc()){
			...
			$courseid = $row['question_paper_id'];
			//echo "courseid:{$courseid}<br>";
			$studid = $row['registration_id'];
			$candidateId = $row['id'];
		}
	}else{
		 $msg = "Candiatedate Answer sheet not uploaded";
		 $_SESSION['msg'] = $msg;
   ...
   //echo "courseid:{$courseid}<br>";
	...
	//fetch coursename
	$result = $conn->query("SELECT `question_paper`,`id` FROM `pe_question_paper` WHERE `id`='{$courseid}'");
	if ($result->num_rows > 0) {
		if($row = $result->fetch_assoc()){
			
			$coursename = $row['question_paper'];
            $QpId = $row['id'];
			//echo "courseid:{$courseid}<br>";
			//$studid = $row['StudId'];
		}
	...
	// echo "studid:{$_SESSION['studid']}<br>qpid:{$_SESSION['QpId']}";
	//close conn
	//$conn->close();
?>
<html>
	<head>
		...
		<link rel="stylesheet" type="text/css" href="<?php echo BASE_URL?>assets/css/teacherStyle.css" />
		<link rel="stylesheet" type="text/css" href="<?php echo BASE_URL?>assets/css/CommonStyle.css" />
		
		<!-- Scripts -->
		<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.1.1/jquery.min.js"></script>
	
		<script type="text/javascript" src="<?php echo BASE_URL?>assets/js/canvasScript.js" ></script>	
		<script type="text/javascript" src="<?php echo BASE_URL?>assets/js/zoomScript.js" ></script>
		<script type="text/javascript" src="<?php echo BASE_URL?>assets/js/mysqlSafe.js" ></script>
		<script>
			$(document).keydown(
				function(e)
				{    
	...
	<body >
		...
		<div class="header-div" style="">
			...
			<p style="font-size:18px;float:right;margin-bottom:0px;margin-right:120px;display:inline-block;margin-top:15px;" >Hello <?php echo $_SESSION['invigilator_name'];?>!</p>
		</div>
		...
		<script type="text/javascript" src="<?php echo BASE_URL?>assets/js/modalScript.js"></script>
		...
		<div id="ans-script-div" style="">
			...
			<?php //php to fetch imgsrcs from db
				...
				$result = $conn->query("SELECT * FROM `pe_candidate_answers` where registration_id=".$_GET['hidid']." ");
			 
                if ($result->num_rows > 0) {
                    echo "<script>"
						."";
					while($row = $result->fetch_assoc()) {
						echo "imgsrcs.push('{$row['corrected_answer']}');";
						echo "originalimgsrcs.push('{$row['uploaded_answer']}');";
						echo "imgids.push('{$row['id']}');";
					}
					echo "</script>";
				} else {
					echo "0 results";
			...
			<div id="ans-script-image-div" style="display:inline-block;" >
				<img id="ans-script-img" src="<?php echo BASE_URL?>admin/assets/uploads/img3.jpg" 
					style="width:800px;height:772px; " 
					onclick=zoomIn(this); />
			...
			<div id="original-ans-script-image-div" style="display:none;" >
				<img id="original-ans-script-img" src="<?php echo BASE_URL?>admin/assets/uploads/img3.jpg" 
			...
			<div id="ans-script-canvas-div" style="display:none" >
				...
				<img id= "img-for-canvas" src="<?php echo BASE_URL?>admin/assets/uploads/img3.jpg" style="width:30px;height:40px;visibility:hidden;" />
				...
				<canvas id="canvas-id" width="500
				" height="568" style="margin-left:15%;border:2px solid;display:inline-block;" >
			...
			<div id="ansScript-controls-div" style="overflow:auto;" >
				...
				<button id="view-original-button" class="round-edge-button green-button move" style="margin-left:4%;margin-top:10px;padding:8px 16px;" >View Original</button>
		...
		<script>
					...
					$('#canvas-toggle-button').on('click',function(){
						if($('#ans-script-image-div').css('display') != 'none' ){
							...
							$('#canvas-toggle-button').html('Evaluate');
		...
		<div id="marks-entry-div" style="">
			...
			<div id="marks-entry-table-div" style="" >
				...
				<table> <!-- Table to display courseid and HidId -->
					<tr>
						<td>Hidden Id: <b><?php echo base64_encode($HidId); ?></b></td>
						<td>Course Id: <b><?php echo $coursename; ?></b></td>
					</tr>
				</table>
				
				<!-- <form id="marks-form" action="submitMarks.php" method="post" > -->
				<form id="marks-form" action="submitMarks.php" method="post" onsubmit="document.getElementById('correction-percent-hidden').value = document.getElementById('correction-percent-text-field').value;">
                <input type="hidden" id="correction-percent-hidden" name="correction_percent" value="0% corrected" />
			...
			<script src="<?php echo BASE_URL?>assets/js/totalMarksCalculation.js" >
		...
		<div id="status-div" style="float:left;width:100%;height:30px;background-color:#a6a6a6;overflow:auto;">
			...
			<script src="<?php echo BASE_URL?>assets/js/progressBarJS.js">
			</script>
