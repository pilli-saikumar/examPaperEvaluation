	...
	foreach($marksids as $marksTBid){
		...
		if($_POST[$marksTBid] !== '') {
			...
			if($correction_percent == 100){
				...
				$updateinvigilator_data = $conn->query("UPDATE `invigilator_evaluated` SET `evaluation_endtime` = '{$evaluation_endtime}' WHERE `invigilator_id` = '{$factid}' AND `candidate_id` = '{$studid}'");
			   }else{
				   $status = 'in_process';
			   }
            if($check->num_rows > 0) {
